...
(function( $ ) {
    ...
    $.redux.initEvents = function() {
        ...
        $( '.redux-action_bar input' ).one(
            'click', function( e ) {
                ...
                } else if( $( this ).attr( 'name' ) == 'redux_save' ) {
					$.redux.getValues();
					$.redux.cleanupNameAtts();
					$( '[name="redux_save"]' ).hide().parent().prepend('<span class="button button-primary">Saving</span>');
		...
		$('body').on('click','#redux-import',function() {
			$.redux.cleanupNameAtts();
		});
    };
	...
	$.redux.cleanupNameAtts = function() {
            ...
            function() {
                ...
                el.find( '#' + relid + '_section_group' ).fadeIn(
                    200, function() {
                        ...
                        $.redux.initFields();
                        $.redux.initNavTabs('#' + relid + '_section_group');
                    }
                );
	...
	$.redux.initNavTabs = function(el) {
//		console.log($el);
		if(!$(el)) { return false; }
		
		var navTabsHTML = '';
		
		if($(el).find('.redux-info-desc').length > 0) {
			$(el).find('.redux-info-desc').each(function() {
				var $self = $(this),
				text = $self.text(),
				id = $self.parent().attr('id') ? $self.parent().attr('id') : false;

				if(text != '' && id) {
					navTabsHTML += '<li><a href="#'+id+'">'+text+'</a></li>';
				}
			});
			
			if(navTabsHTML != '') {
				navTabsHTML = '<ul>'+navTabsHTML+'</ul>';
			}
		}

		if($(el).parents( '.redux-container:first' ).find( '.dfd-nav-tabs-section' ).length > 0) {
			$(el).parents( '.redux-container:first' ).find( '.dfd-nav-tabs-section' ).html(navTabsHTML);

			$('body').on('click', '.dfd-nav-tabs-section a', function(e) {
				if(typeof $.fn.scrollTo != undefined) {
					e.preventDefault();
					$(window).scrollTo($(this).attr('href'), {duration:'slow',offset: -100});
				}
			});
		}
	};
    ...
    $.redux.stickyInfo = function() {
        var stickyWidth = $( '.redux-main' ).innerWidth() - 1;
