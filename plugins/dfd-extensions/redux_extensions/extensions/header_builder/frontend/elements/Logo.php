...
class DfdHeaderBuilder_Logo extends DfdHeaderBuilderElementAbstract {
	...
	public function render() {
		...
		echo '<a href="' . esc_url(apply_filters('dfd_header_builder_logo_url', home_url())) . '" title="' . esc_attr__('Site logo', 'dfd-native') . '">';
		if ($view == "desktop") {
			$this->getLogoImg();
		} else {
	...
	protected function getLogoImg($class = 'main-logo') {
		...
		if ($img && is_array($img)) {
			$logos_data_atts .= ' width="'. esc_attr($img[1]).'"';
			$logos_data_atts .= ' height="'. esc_attr($img[2]).'"';
			$img = $img[0];
		}
	...
	private function main_logo_img($class = 'main-logo') {
		...
		if (isset($header_logo['url']) && !empty($header_logo['url'])) {
			...
			if(!empty($header_logo['width'])) {
				$logos_data_atts .= ' width="' . esc_attr($header_logo['width']) . '"';
			}
			
			if(!empty($header_logo['height'])) {
				$logos_data_atts .= ' height="' . esc_attr($header_logo['width']) . '"';
			}
	...
	private function sticky_logo_html() {
		...
		. '<a href="' . esc_url(apply_filters('dfd_header_builder_sticky_logo_url', home_url())) . '" title="' . esc_attr__('Site logo', 'dfd-native') . '">';
