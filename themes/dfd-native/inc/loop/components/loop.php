...
//			}
		...
		function build_loop_html() {
			...
			if(is_archive() && isset($dfd_native['post_archive_description']) && $dfd_native['post_archive_description'] == 'on') {
				$category = get_the_category();
				if(!empty($category[0]->description)) {
					echo '<div class="dfd-archive-page-description">'.$category[0]->description.'</div>';
				}
			}
